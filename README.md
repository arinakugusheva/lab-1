...
Дисциплина: Теория алгоритмов Направление подготовки: 38.03.05 «Бизнес-информатика» Курс: 2 Объём: 4 академических часа Язык реализации: Python 3.10+
...
Изучить понятия итерационного процесса, начального приближения, критерия остановки и скорости сходимости.
Реализовать алгоритмы вычисления $\sqrt{a}$ и $\sqrt[3]{b}$ методом Ньютона, а также $\ln c$ методом разложения в степенной ряд.
Экспериментально определить число итераций, необходимое для достижения заданной точности $\varepsilon$.
Сравнить полученные значения с эталонными значениями стандартной библиотеки (math) и оценить абсолютную погрешность.
...
Изучить теоретическую часть и рекомендуемую литературу.
Построить блок-схемы алгоритмов вычисления $\sqrt{a}$, $\sqrt[3]{b}$ и $\ln c$.
Реализовать алгоритмы в виде отдельных функций на языке Python. Не использовать math.sqrt, math.log, **0.5 и аналоги внутри вычислительных функций; допускается их применение только для получения эталонных значений.
Каждая функция должна возвращать пару «результат, число итераций».
Выполнить расчёты для исходных данных своего варианта (табл. 1) и дополнительного требования (табл. 2).
Сравнить результаты с эталоном и заполнить таблицу результатов.
...
Титульный лист.
Цель работы и формулировку задания своего варианта.
Краткие теоретические сведения: итерационные формулы с пояснениями.
Блок-схемы алгоритмов (ГОСТ 19.701-90 или нотация Mermaid/draw.io).
Листинг программы с комментариями.
Результаты расчётов в виде таблицы и, при необходимости, графики.
...
Все три алгоритма достигли заданной точности $\varepsilon = 10^{-6}$. Фактическая погрешность методов Ньютона оказалась на много порядков меньше $\varepsilon$ ($\approx 10^{-15}$). Это объясняется квадратичной сходимостью: к моменту выполнения критерия по разности погрешность уже пренебрежимо мала.
Для $\sqrt[3]{10}$ потребовалось больше итераций, чем для $\sqrt{2}$. Причина — более удалённое начальное приближение ($x_0 = 10$ при $x^* \approx 2{,}15$): на первых шагах метод сходится практически линейно, а квадратичный режим наступает лишь вблизи корня.
Разложение в ряд для $\ln 5$ ($\lvert y \rvert = 2/3$) сходится линейно: 14 членов ряда. Погрешность того же порядка, что и $\varepsilon$. Это характерно для процессов первого порядка.
Для повышения эффективности вычисления логарифма больших аргументов целесообразно применять нормализацию $c = m \cdot 2^k$ (см. требование Д3).
7. Контрольные вопросы
Дайте определение итерационного алгоритма. Какими свойствами алгоритма (дискретность, детерминированность, результативность, массовость) он обладает и как обеспечивается результативность?
Выведите итерационную формулу Герона из метода Ньютона.
Чем отличаются критерии остановки «по разности» и «по невязке»? В каких случаях они могут давать разные результаты?
Что такое порядок сходимости? Почему метод Ньютона сходится быстрее метода половинного деления?
Как влияет выбор начального приближения на число итераций? Приведите пример из собственных расчётов.
Почему ряд $\ln(1 + x)$ не подходит для вычисления $\ln 100$, а ряд с $y = (c - 1)/(c + 1)$ подходит?
В чём смысл нормализации аргумента $c = m \cdot 2^k$?
Почему фактическая погрешность может быть значительно меньше $\varepsilon$? Может ли она оказаться больше $\varepsilon$?
Зачем в итерационном алгоритме нужно ограничение $N_{\max}$?
Приведите пример задачи из экономики или бизнес-аналитики, решаемой итерационно (например, расчёт внутренней нормы доходности IRR).
8. Рекомендуемая литература
Кнут Д. Э. Искусство программирования. Т. 1. Основные алгоритмы. — 3-е изд. — М.: Вильямс, 2017.
Вирт Н. Алгоритмы и структуры данных. Новая версия для Оберона. — М.: ДМК Пресс, 2016.
...
Бхаргава А. Грокаем алгоритмы. Иллюстрированное пособие для программистов и любопытствующих. — 2-е изд. — СПб.: Питер, 2024.
